...
class CarouselInputWidget(QWidget):
    ...
    def _previous_value(self):
        ...
        try:
            ...
            if current_index == -1 and self._allowed_values: # If current value is not in list, start from last
    ...
    def _next_value(self):
        ...
        try:
            current_index = self._allowed_values.index(self.value()) if self.value() in self._allowed_values else -1
            if current_index == -1 and self._allowed_values: # If current value is not in list (e.g. None), start from first
                new_index = 0
                ...
                new_index = current_index + 1
            self.setValue(self._allowed_values[new_index])
        except (ValueError, IndexError): # ValueError if not found, IndexError if list is empty after check
