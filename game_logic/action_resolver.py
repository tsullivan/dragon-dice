...
class ActionResolver(QObject):
    ...
    def determine_defending_army_identifier(
        self, defending_player_name: str, combat_location: str = None
    ) -> str:
        """
        ...
        if combat_location or self._current_combat_location:
            ...
            armies_at_location = self.game_state_manager.get_all_armies_at_location(
                defending_player_name, location
            )

            if len(armies_at_location) == 1:
                # Only one army at location, that's the target
                army_data = armies_at_location[0]
                return army_data.get(
                    "unique_id", f"{defending_player_name}_{army_data['army_type']}"
                )
                ...
                for army_type in priority_order:
                    for army_data in armies_at_location:
                        if army_data["army_type"] == army_type:
                            return army_data.get(
                                "unique_id", f"{defending_player_name}_{army_type}"
                            )

        # Fallback to active army
        active_army_type = self.game_state_manager.get_active_army_type(
            defending_player_name
        )
        if active_army_type:
            return self.game_state_manager.generate_army_identifier(
                defending_player_name, active_army_type
            )

        # Final fallback to home army
        return self.game_state_manager.generate_army_identifier(
            defending_player_name, "home"
        )

    def determine_attacking_army_identifier(
        self, attacking_player_name: str, combat_location: str = None
    ) -> str:
        """
        ...
        return self.determine_defending_army_identifier(
            attacking_player_name, combat_location
        )
    ...
    def resolve_melee_attack(
        ...
        print(
            f"ActionResolver: Resolving melee attack between {attacking_player_name} and {defending_player_name}."
        )
        print(f"ActionResolver: Attacker roll results: {attacker_roll_results_str}")

        # Step 1: Parse attacker's dice results
        parsed_attacker_dice = self.parse_dice_string(
            attacker_roll_results_str, "MELEE"
        )
        ...
        attacker_outcome = self.process_attacker_melee_roll(
            attacking_player_name, parsed_attacker_dice
        )
        ...
        print(
            f"ActionResolver: Attacker scored {attacker_outcome['hits']} hits. Awaiting defender's save roll."
        )
        ...
        for _ in range(id_icons_to_convert):
            for unit in attacking_units:
                ...
                if unit_id not in units_that_used_id:
                    ...
                    unit_def = (
                        self.game_state_manager.unit_roster.get_unit_definition(
                            unit.get("unit_type", "")
                        )
                    ...
                    print(
                        f"ActionResolver: Unit {unit.get('name', 'Unknown')} used ID for {default_id_melee} melee."
                    )
    ...
    def process_defender_save_roll(
        ...
        if bullseye_active:
            print(
                "ActionResolver: Attacker Bullseye is active, preventing ID conversion to saves."
            )
        ...
        for _ in range(id_icons_to_convert):
            ...
            for unit in defending_units:
                ...
                if unit_id not in units_that_used_id:
                    ...
                    units_that_used_id.add(unit_id)
                    print(
                        f"ActionResolver: Unit {unit.get('name', 'Unknown')} used ID for {default_id_saves} save."
                    )
                    break  # Move to the next ID icon to be converted
        ...
        for icon_data in parsed_save_dice:
            if icon_data.get("type") == constants.ICON_SAI:
                ...
                if sai_type == "SHIELD":  # Placeholder SAI name
                    ...
                    print(
                        f"ActionResolver: SAI Shield generated {count} additional saves"
                    )
        ...
        if final_damage > 0:
            ...
            print(
                f"ActionResolver: Applied {final_damage} damage to {defending_player_name}'s army"
            )
        ...
        if final_damage < attacker_hits:  # Some damage was saved
            ...
            defender_melee_icons = sum(
                item.get("count", 0)
                for item in parsed_save_dice
                if item.get("type") == constants.ICON_MELEE
            )
            # Check if defender still has units capable of counter-attacking
            surviving_units = [
                unit for unit in defending_units if unit.get("health", 0) > 0
            ]
    ...
    def parse_dice_string(self, dice_string: str, roll_type: str) -> list:
        ...
        icon_mappings = {
            # Full names
            "melee": constants.ICON_MELEE,
            "missile": constants.ICON_MISSILE,
            "magic": constants.ICON_MAGIC,
            "save": constants.ICON_SAVE,
            "id": constants.ICON_ID,
            "sai": constants.ICON_SAI,
            "maneuver": constants.ICON_MANEUVER,
            # Common abbreviations
            "m": constants.ICON_MELEE,
            "mel": constants.ICON_MELEE,
            "mi": constants.ICON_MISSILE,
            "mis": constants.ICON_MISSILE,
            "mag": constants.ICON_MAGIC,
            "sv": constants.ICON_SAVE,
            "s": constants.ICON_SAI,  # When not followed by 'ai'
            "man": constants.ICON_MANEUVER,
            # Dragon-specific icons
            "claw": constants.ICON_DRAGON_ATTACK_CLAW,
            "bite": constants.ICON_DRAGON_ATTACK_BITE,
            "tail": constants.ICON_DRAGON_ATTACK_TAIL,
            "breath": constants.ICON_DRAGON_BREATH,
        }

        # Valid SAI types
        valid_sais = {
            "bullseye": constants.SAI_BULLSEYE,
            "doubler": constants.SAI_DOUBLER,
            "tripler": constants.SAI_TRIPLER,
            "recruit": constants.SAI_RECRUIT,
            "magic_bolt": constants.SAI_MAGIC_BOLT,
        }
        ...
        for part in parts:
            ...
            try:
                ...
                if match:
                    ...
                    if not icon_type:
                        print(
                            f"ActionResolver: Warning - Unknown icon type '{icon_name}' in '{part}'"
                        )
                        continue
                    ...
                    if icon_type == constants.ICON_SAI and sai_type:
                        sai_constant = valid_sais.get(sai_type)
                        if sai_constant:
                            result_dict["sai_type"] = sai_constant
                        else:
                            print(
                                f"ActionResolver: Warning - Unknown SAI type '{sai_type}' in '{part}'"
                            )
                            continue
                    ...
                    if sai_match:
                        ...
                        print(
                            f"ActionResolver: Warning - Could not parse dice part '{part}'"
                        )
        ...
        print(
            f"ActionResolver: Parsed dice string into {len(parsed_list)} results: {parsed_list}"
        )
        return parsed_list

    def resolve_defender_save_response(
        self, defending_player_name: str, save_roll_results_str: str
    ):
        ...
        print(
            f"ActionResolver: Processing defender save response: {save_roll_results_str}"
        )
        ...
        self.action_resolved.emit(
            {
                ...
                "counter_attack_possible": save_outcome.get(
                    "counter_attack_possible", False
                ),
            }
        )
    ...
    def resolve_missile_attack(
        ...
        print(
            f"ActionResolver: Resolving missile attack from {attacking_player_name} to {defending_player_name}"
        )

        # Parse missile dice results
        parsed_missile_dice = self.parse_dice_string(
            missile_roll_results_str, "MISSILE"
        )

        # Calculate missile hits (similar to melee but no save phase)
        missile_hits = 0
        for icon_data in parsed_missile_dice:
            if icon_data.get("type") == constants.ICON_MISSILE:
    ...
    def resolve_magic_action(
        self, casting_player_name: str, magic_roll_results_str: str
    ):
    ...
    def resolve_maneuver_action(
        self, maneuvering_player_name: str, maneuver_roll_results_str: str
    ):
        """Resolves a maneuver action (movement and positioning)."""
        print(
            f"ActionResolver: Resolving maneuver action for {maneuvering_player_name}"
        )

        # Parse maneuver dice results
        parsed_maneuver_dice = self.parse_dice_string(
            maneuver_roll_results_str, "MANEUVER"
        )
        ...
        for icon_data in parsed_maneuver_dice:
            if icon_data.get("type") == constants.ICON_MANEUVER:
        ...
        if maneuver_successes > 0:
            ...
            print(
                f"ActionResolver: {maneuvering_player_name} can perform {maneuver_successes} movement actions"
            )
    ...
    def resolve_counter_attack(
        ...
        parsed_counter_dice = self.parse_dice_string(
            counter_attack_roll_results_str, "MELEE"
        )

        # Process counter-attack (simplified - no saves for counter-attacks)
        counter_hits = 0
        for icon_data in parsed_counter_dice:
            if icon_data.get("type") == constants.ICON_MELEE:
        ...
        if counter_hits > 0:
            ...
            original_attacker_army_id = self.determine_attacking_army_identifier(
                original_attacker_name
            )
            self.game_state_manager.apply_damage_to_units(
                original_attacker_name, original_attacker_army_id, counter_hits
            )
            print(
                f"ActionResolver: Counter-attack dealt {counter_hits} damage to {original_attacker_name}"
            )
