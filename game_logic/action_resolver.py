...
class ActionResolver(QObject):
    ...
    def resolve_melee_attack(
        self, attacking_player_name: str, defending_player_name: str, attacker_roll_results_str: str
    ):
        ...
        for _ in range(id_icons_to_convert):
            for unit in attacking_units:
                ...
                if unit_id not in units_that_used_id:
                    ...
                    unit_def = self.game_state_manager.unit_roster.get_unit_definition(unit.get("unit_type", "")) if hasattr(self.game_state_manager, 'unit_roster') else None
    ...
    def process_defender_save_roll(
        ...
        if final_damage < attacker_hits:  # Some damage was saved
            ...
            defender_melee_icons = sum(
                item.get("count", 0) for item in parsed_save_dice 
            ...
            if defender_melee_icons > 0 and surviving_units:
                ...
                print(f"ActionResolver: Counter-attack possible! {defender_melee_icons} melee icons, {len(surviving_units)} surviving units")
    ...
    def resolve_defender_save_response(self, defending_player_name: str, save_roll_results_str: str):
        ...
        save_outcome = self.process_defender_save_roll(
            defending_player_name, 
            parsed_save_dice, 
            self._pending_attacker_outcome
        )
    ...
    def resolve_missile_attack(self, attacking_player_name: str, defending_player_name: str, missile_roll_results_str: str):
    ...
    def resolve_counter_attack(self, counter_attacking_player_name: str, counter_attack_roll_results_str: str, original_attacker_name: str):
        """Resolves a counter-attack following a successful save."""
        print(f"ActionResolver: Resolving counter-attack from {counter_attacking_player_name} against {original_attacker_name}")
