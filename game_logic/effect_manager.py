...
class EffectManager(QObject):
    ...
    def get_displayable_effects(self) -> list[str]:
        ...
        return [
            f"Effect: {effect['description']} on {effect['target_identifier']}"
    ...
    def get_active_modifiers(
        ...
        for effect in self.active_effects:
            ...
            if effect.get("affected_player_name") == target_player_name:  # Basic check
                if (
                    "melee results -3" in effect.get("description", "").lower()
                    and action_type == "MELEE"
                ):
                    modifiers["melee_bonus"] -= 3
                elif (
                    "all results halved" in effect.get("description", "").lower()
                ):  # General halving
                    modifiers["halve_results"] = True
                elif (
                    "melee results halved" in effect.get("description", "").lower()
                    and action_type == "MELEE"
                ):
                    modifiers["halve_results"] = True
        ...
        print(
            f"EffectManager: Modifiers for {target_player_name} ({action_type}): {modifiers}"
        )
        return modifiers
