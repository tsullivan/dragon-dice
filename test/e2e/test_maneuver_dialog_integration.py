...
class TestManeuverDialogIntegration(unittest.TestCase):
    ...
    def _setup_tracking(self):
        ...
        self.engine.counter_maneuver_requested.connect(
            lambda location, armies: self.signals_received.append(f"counter_maneuver_requested: {location}, armies: {len(armies)}")
        )
    ...
    def test_e2e_maneuver_dialog_counter_maneuver_signal_flow(self):
        ...
        with patch('views.maneuver_dialog.CounterManeuverDecisionDialog.exec') as mock_exec:
        ...
        self.assertTrue(any("counter_maneuver_requested" in signal for signal in self.signals_received))
        ...
        counter_signal = [s for s in self.signals_received if "counter_maneuver_requested" in s][0]
    ...
    def test_e2e_maneuver_dialog_handles_no_opposition(self):
        ...
        isolated_setup = [
            {
                ...
                "armies": {
                    "home": {
                        ...
                        "units": [
                            {"name": "Soldier", "health": 1, "max_health": 1, "unit_id": "home_1", "unit_type": "amazon_soldier"},
                        ],
        ...
        isolated_engine.counter_maneuver_requested.connect(
            lambda location, armies: isolated_signals.append(f"counter_maneuver_requested: {location}")
        )
        isolated_engine.terrain_direction_choice_requested.connect(
            lambda location, face: isolated_signals.append(f"terrain_direction_requested: {location}")
        )
        ...
        with patch('views.maneuver_dialog.TerrainDirectionDialog.exec') as mock_terrain_exec:
        ...
        counter_signals = [s for s in isolated_signals if "counter_maneuver_requested" in s]
        self.assertEqual(len(counter_signals), 0)
        
        # Should have terrain direction request (automatic success)
        terrain_signals = [s for s in isolated_signals if "terrain_direction_requested" in s]
