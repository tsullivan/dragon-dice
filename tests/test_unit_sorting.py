    ...
    if "Amazon" in units_by_species:
        ...
        for i in range(len(sorted_units) - 1):
            ...
            if current_health == next_health:
                ...
                assert current_class <= next_class, f"Class type sorting violation at position {i}: {current_class} > {next_class}"
                ...
                if current_class == next_class:
                    ...
                    assert current_name <= next_name, f"Name sorting violation at position {i}: {current_name} > {next_name}"
